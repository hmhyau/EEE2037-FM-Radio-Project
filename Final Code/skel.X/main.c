...
unsigned int ManualChan(unsigned int channel, unsigned char dir) {
    ...
    //if(FMread(2, &channel) != XS) return XF;
    //channel = channel >> 7 + 690;
    //channel += 690;
...
void showVol(unsigned int vol) {       //TODO
    int ones, tens;
    int i, j;
    clrscn();
    ones = vol % 10;
...
/*
 * Increase/decrease volume by one step depending on button input.
 *
 */
unsigned int Vol_change(unsigned int vol, unsigned char dir){ //should be correct
    switch (dir) {
        case TRUE:
            if(vol < 18){
                vol++;
...
//    showFreq(1888);
    ...
    for(;;){
		...
		switch (evt) {
            ...
            case 5 : vol = Vol_change(vol, TRUE); dly(1000); showFreq(frequency); break;
            case 6 : vol = Vol_change(vol, FALSE); dly(1000); showFreq(frequency); break;
